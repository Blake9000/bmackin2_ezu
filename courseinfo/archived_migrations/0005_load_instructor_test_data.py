# Generated by Django 5.2.8 on 2026-04-06 02:54
...
def remove_instructor_data(apps, schema_editor):
    ...
    for instructor in INSTRUCTORS:
        try:
            instructor_object = instructor_class.objects.using(db_alias).get(
                first_name=instructor['first_name'],
                last_name=instructor['last_name'],
                disambiguator=instructor['disambiguator']
            )
            ...
            for model in apps.get_models():
                for field in model._meta.fields:
                    if getattr(field, 'remote_field', None) and field.remote_field.model == instructor_class:
                        model.objects.using(db_alias).filter(
                            **{field.name: instructor_object}
                        ).delete()

            instructor_object.delete()

        except ObjectDoesNotExist:
            print('Instructor entry not found during rollback:', instructor['first_name'], instructor['last_name'])

class Migration(migrations.Migration):

    dependencies = [
        ('courseinfo', '0004_alter_instructor_options_alter_registration_options_and_more'),
    ]
